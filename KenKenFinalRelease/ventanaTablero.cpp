...
: TForm(Owner)
{
	...
	clues=Form1->nivel;
	B4->Caption="Revelar ["+AnsiString(clues)+"]";
}
//----------------------------------------------------------------------------
...
void __fastcall TForm2::B1Click(TObject *Sender)
{
	...
	for(int i=0;i<Form1->nivel;i++)
	{
		...
		{
			if(SG1->Cells[i][j].IsEmpty() || SG1->Cells[i][j]!=AnsiString(Form1->tablero[i][j]))
			{
				check=false;
				i=Form1->nivel;
				j=Form1->nivel;
...
void __fastcall TForm2::B4Click(TObject *Sender)
{
	if (fl1)
	{
		if(clues)
		{
			...
			}while(SG1->Cells[randNum1][randNum2]==AnsiString(Form1->tablero[randNum1][randNum2]));
...
void __fastcall TForm2::B3Click(TObject *Sender)
{
	...
	ofstream hiRes;
	hiRes.open("data\\hiRes.txt");
	for(int i=0;i<Form1->cantAgrup;i++)
	{
		hiRes <<Form1->results[i]<<" ";
	}
	hiRes.close();
	ofstream hiOps;
	hiOps.open("data\\hiOps.txt");
	for(int i=0;i<Form1->cantAgrup;i++)
	{
		hiOps <<Form1->operations[i]<<" ";
	}
	hiOps.close();

	ofstream info;
	info.open("data\\info.txt");
	info << Form1->nivel << endl << Form1->cantAgrup << endl << s << endl << m << endl << clues;
	info.close();
	Close();
	ShowMessage("Partida guardada, puede cargarla nuevamente desde la opción \"Continuar Guardada\".");
}
//---------------------------------------------------------------------------
