...
void __fastcall TForm1::T11Click(TObject *Sender)
{
	...
	for(int i=0;i<nivel;i++) //Creacion aleatoria de agrupaciones
	{
		for(int j=0;j<nivel;j++)
		{
			if(agrup[i][j]==0)
	...
	for(int i=0;i<cantAgrup;i++) //Creacion de colores y operaciones aleatorias para grupos
	{
		results[i]=0;
		if(cantAgrup<50)
	...
	for(int k=0;k<cantAgrup;k++)  //Creacion de resultado de las agrupaciones
	{
